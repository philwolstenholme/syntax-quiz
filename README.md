...
- [What is Syntax Quiz?](#what-is-syntax-quiz)
- [Question Design and Difficulty Levels](#question-design-and-difficulty-levels)
- [The Feedback Banner](#the-feedback-banner)
- [The Skip Button: No-Pressure Learning](#the-skip-button-no-pressure-learning)
- [Hints](#hints)
- [Scoring and Streak System](#scoring-and-streak-system)
- [Retry Rounds](#retry-rounds)
- [Save and Resume](#save-and-resume)
- [API](#api)
- [Accessibility and Sensory Feedback](#accessibility-and-sensory-feedback)
...
The quiz has a REST API that lets you browse questions and play the game programmatically. Interactive documentation is available at [syntax-quiz.netlify.app/api](https://syntax-quiz.netlify.app/api), where you can explore the endpoints and test requests directly from the browser.

### Reference endpoints

- **GET /api/levels** — List all difficulty levels and their question counts.
- **GET /api/questions** — Retrieve all questions for a given level, including correct answers, hints, and explanations.

### Playing via the API

The play endpoints let you complete an entire game through the API without a browser UI. The game is stateless on the server — all state is carried in an opaque `gameState` token that you receive in each response and send back with your next request.

1. **Start a game** — `POST /api/play/start` with a level (1, 2, or 3). You receive the first question, a `gameState` token, and your initial progress.
2. **Answer or skip** — `POST /api/play/answer` with the `gameState` token and your answer (or `null` to skip). You receive feedback on whether you were correct, your updated score, and the next question.
3. **Repeat** until the response includes `complete: true`.

Correct answers earn points with a streak multiplier — consecutive correct answers increase the multiplier. Wrong answers and skips reset the streak and queue the question for a retry round. After all questions have been attempted, any missed questions are reshuffled into a retry round that must be completed before the game ends.
