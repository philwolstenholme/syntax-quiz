...
export const ScorePage = () => {
  ...
  // Check if access is valid based on the URL params
  const isValidAccess = level && completed && totalQuestions > 0;

  // Track if we've already redirected to prevent loops
  const hasRedirected = useRef(false);

  useEffect(() => {
    // Redirect if access is not valid and we haven't already redirected
    if (!isValidAccess && !hasRedirected.current) {
  ...
  const handleRestart = () => {
    setLocation(`/syntax-quiz/level/${levelId}/questions`);
  };
