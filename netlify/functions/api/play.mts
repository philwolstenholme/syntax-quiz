...
const SET_COOKIE_HEADER = {
  "Set-Cookie": {
    ...
    description:
      "Sets the `gameState` cookie (Path=/api/play; SameSite=Strict). Cleared when the game is complete.",
  },
};
...
const GAME_STATE_COOKIE_PARAM = {
  ...
  schema: { type: "string" as const },
  description:
    "The game state token, automatically sent by the browser if the cookie was set by a previous response.",
};
...
function withSetCookieHeader(current: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [status, res] of Object.entries(responses)) {
    ...
    patched[status] = {
      ...response,
      headers: { ...((response.headers as Record<string, unknown>) ?? {}), ...SET_COOKIE_HEADER },
    };
  }
...
const start = playOs
  ...
  .output(
    z.object({
      gameState: z
        .string()
        .describe("Opaque token encoding the game state — send this back with your next answer"),
  ...
  .route({
    ...
    spec: (current) => {
      ...
      return {
        ...patched,
        parameters: [
          ...(((current as Record<string, unknown>).parameters as unknown[]) ?? []),
          GAME_STATE_COOKIE_PARAM,
        ],
      } as typeof current;
    },
  })
  .input(
    z.object({
      gameState: z
        ...
        .optional()
        .describe(
          "The opaque game state token from the previous response. Optional when playing via the browser — the cookie is used automatically.",
        ),
      answer: z
        .string()
        .nullable()
        .describe(
          "Your answer choice (must match one of the provided answers exactly), or null to skip",
        ),
    }),
  )
  .output(
    z.object({
      gameState: z
        ...
        .nullable()
        .describe(
          "Updated game state token for the next request, or null when the game is complete",
        ),
      feedback: FeedbackSchema.describe("Feedback on the question you just answered"),
      question: PlayQuestionSchema.nullable().describe(
        "The next question, or null when the game is complete",
      ),
  ...
  .handler(async ({ input, context }) => {
    ...
    if (!gameStateToken) {
      throw new ORPCError("BAD_REQUEST", {
        message:
          "Missing game state — provide gameState in the request body or start a game first to set the cookie",
      });
    }
